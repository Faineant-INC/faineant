...
insert into auth.users (id, email, raw_user_meta_data) values
  ...
  ('aaaa0000-0000-0000-0000-0000000000e3','d3@example.com','{"role":"PROVIDER","first_name":"Pending","last_name":"Provider"}');
...
update public.provider_profiles set latitude = 41.8781, longitude = -87.6298
  where user_id = 'aaaa0000-0000-0000-0000-0000000000e3';
insert into public.services (provider_profile_id, name, category, duration_minutes, price_in_cents, is_active)
  ...
  where user_id in (
    'aaaa0000-0000-0000-0000-0000000000e1',
    'aaaa0000-0000-0000-0000-0000000000e2',
    'aaaa0000-0000-0000-0000-0000000000e3'
  );
...
select is(
  ...
  2, 'category filter returns only approved HAIRCUT providers');
select is(
...
select is(
  (select count(*)::int from public.get_provider_busy_intervals(
      (select id from public.search_providers('Near') limit 1),
      '2026-08-01T00:00:00Z','2026-08-02T00:00:00Z')),
  0, 'no busy intervals for a provider with no bookings');
select is(
  (select count(*)::int from public.search_providers('Pending')),
  0, 'pending providers are absent from public search');
select is(
  (select count(*)::int from public.services),
  2,
  'pending provider services are absent from the anonymous catalog'
);
reset role;
